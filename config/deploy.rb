...
set(:application) do
  current_app   = 'staging'
  app_name = Capistrano::CLI.ui.ask "Deploy To: [#{current_app}]: "
...
set :deploy_to,  "/var/www/apps/dcs/#{application}" 
set :domain,     "74.207.254.116"                
set :url,        "#{domain}"     
...
set :remote_db_name, "siteforcare_production"
...
#############################################################
set :scm,              :git          
set :github_user_repo, 'jwachira'
set :github_project_repo, 'dcs'
...
namespace :deploy do
  ...
  task :install_gems, :roles => [:app] do
    run "cd #{release_path}; /usr/local/ruby-enterprise/bin/bundle install"
    run "cd #{release_path}; sudo #{rake_path} RAILS_ENV=#{rails_env} db:create"
